...
To this end we formulate the Relational-To-Ontology Mapping Problem (\relonto{}) as a Minimum Steiner Tree Problem (STP) with side constraints \cite{deuna2016steiner}.
...
We choose a search strategy that will first try to match attributes using the cheapest edges in $\mathcal{M}_\mathcal{O}^{s^\star}$. Then, it will try to use the cheapest unfixed edge $e\in\mathcal{E_O}$ that completes a pattern $p$ such that $w_\mathcal{P}(p) > w_\mathcal{O}(e)$ or simply the cheapest edge if none complete a pattern.
