...
\tikzstyle{hasaA} = [>=stealth',draw,dashed,-,black!70]
...
focus on a specific data integration problem: automatically mapping relational 
data sources onto a user provided ontology.
...
This problem is formulated for an \emph{integration graph} which includes the 
attributes from the new source as well as ontology classes and properties. We
apply machine learning techniques to assign costs to the links in the 
constructed integration graph.
...
\begin{tikzpicture}
	...
	\path[draw,hasaA,->] (pe) edge [bend left = 10] 
	node [midway, above, sloped] {name} (-0.5,2);
	\path[draw,hasaA,->] (pe) edge [bend left = 0] 
	node [midway, below, sloped] {birthDate} (0,2);	
	
	\path[draw,hasaA,->] (co) edge [bend left = 0] 
	...
	\path[draw,hasaA,->] (ev) edge [bend left = 0] 
	...
	\path[draw,hasaA,->] (pl) edge [bend left = 0] 
...
`\tikz{ \path[hasaA,->] (0,1) -- (3ex,1);}' means ``has a data property'' and 
...
\section{ML for Training on Source Descriptions \ddg{Better title than this 
please...}}
...
\subsection{Frequent Subgraph Pattern Mining}
...
The Steiner Tree Problem (STP) is defined the following way. Given a graph $G =
...
not necessarily rooted directed trees, therefore the directions of the edges is 
ignored for the STP model (and are all treated as undirected edges). 
...
We need to build a subgraph $T^\star= (V^\star, E^\star)$ 
that is a subset of the integration graph 
$\mathcal{I}_\mathcal{O}^{s^\star}$ for the new source $s^\star$. The tree will 
...
\begin{flalign}
	...
	\label{EQ:map}&&\\
	& w_{ADIFF} = \sum_{(a,d) \in \mathcal{M}_\mathcal{O}^{s^\star}} 
	w_\mathcal{I}(~(a,d)~) * \llbracket match[a] = d\rrbracket 
	\label{EQ:matchcost}
\end{flalign}

Eq. \ref{EQ:obj} is the objective function, we are trying to minimize the cost 
of $T^\star$. Eq. \ref{EQ:stp} enforces that the solution $T^\star$ is indeed a 
tree defined by the $c_n$ and $c_e$ variables, subgraph of the alignment graph 
