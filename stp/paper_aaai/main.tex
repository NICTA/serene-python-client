...
description. 
We
use the term ``appropriate'' since there might be many such triples which are well-formed source descriptions, but only one or a few will capture the intended meaning of the source. 
...
The algorithm we used to construct $\mathcal{G_O}$ is the same as \cite{taheriyan2016learning} and is described in detail in their work.
...
Frequent Graph Pattern Mining~\cite{yan2002gspan}. 
...
However, they use approximation algorithms both for the matching and for the STP parts.
...
\end{axis}
\end{tikzpicture}
\caption{Performance of chuffed for museum leave-one-out}\label{fig:loo}
\end{figure}
