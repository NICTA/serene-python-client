...
\newcommand{\serene}{\textsc{•}sc{Serene}}
...
In this paper, we tackle the data integration problem consisting in automatically mapping a new relational data source onto a user provided ontology.
To do so, we develop a new tool that automatically builds a semantic model which describes a relational data source in terms of concepts and relationships defined by an ontology \cite{taheriyan2016learning}.
...
\begin{table}[h]
	...
	\rowcolor{Gray}
	prop-range-format numerical range format  & The proportion of entries which follow some ([0-9]+)-([0-9]+)\\ 
...
% 1) First try using the cheapest matches, until a match is found
