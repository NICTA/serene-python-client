...
\begin{itemize}
	...
	seamlessly incorporates techniques from the areas of machine learning, knowledge representation and constraint programming.
%	\npr{symbolic AI, ontological reasoning, transitive closure}
	...
	reuse and adapt to specific situations. This is achieved since CP allows introduction of additional constraints with no changes to the solver. 
\end{itemize}
...
{$s^\star$:};
	...
	\path[draw] (a4) -- (con2);
\end{tikzpicture}
\caption{Example of integration graph. (`\protect\tikz{ \protect\node[att] () at (0,0)  
...
where $rank_i$ is the rank of the correct semantic label for the attribute $a_i$ among the top $k$ predictions and $n$ is the number of attributes in the data source.
...
\begin{table}[!ht]\small
  \centering
  \vspace*{-0.8em}
  \caption{Average performance of semantic labeling models for leave one out strategy.  \vspace*{-0.8em}}
  	\label{tab:semlab}
...
\chuffed{}, which is the solver of \serene{}, successively finds better solutions in terms of Eq. \ref{EQ:obj}.
However,
...
Furthermore, we have explored how the systems perform if we manually filter out the unmatched attributes.
In such scenario \serene{} also yields better solutions than \karma{}, and in many situations \serene{} can prove optimality of the found solutions.
This observation provides further evidence towards the validity of our new modeling framework.   
...
Our approach is complementary to these approaches and at its current stage is semi-automatic.
However, CP offers a convenient framework to incorporate integrity constraints specified either within relational or ontological schema as additional constraints to govern the search for the solution.
