...
/Author (Diego De Una , Nataliia Ruemmele, Graeme Gange, Peter Schachte and Peter J. Stuckey)
...
\author{Diego De U\~na$^1$ , Nataliia R\"ummele$^2$, Graeme Gange$^1$, Peter Schachte$^1$ \and Peter J. Stuckey$^{1,3}$\\
$^1$Department of Computing and Information Systems --- The University of Melbourne\\
$^2$Siemens, Germany ~~~~~
$^3$Data61, CSIRO, Melbourne, Australia\\
d.deunagomez@student.unimelb.edu.au, nataliia.ruemmele@siemens.com\\ \{gkgange,schachte,pstuckey\}@unimelb.edu.au
}
...
In this paper we improve previous work by \citeasnoun{taheriyan2016learning} using Machine Learning (ML) to take into account inconsistencies in the data (unmatchable attributes) and encode the problem as a variation of the Steiner Tree, for which we use work by \citeasnoun{deuna2016steiner} in Constraint Programming (CP).
...
\node[att] () at (0,-1)  
{};  
\node[text=black,text width=1.5cm] (legend1) at (1.3,-1)  
{\small 
	Attribute nodes};
...
To evaluate our new system \serene{}\footnote{ \url{http://github.com/NICTA/serene-python-client/tree/stp/stp}}, we show that its semantic labeling model 
...
not based on the notion of similarity~\cite{ruemmele2018evaluating}.
...
We have also observed that given enough time \serenepats{} finds much better solutions both in precision and recall.
These observations provide further evidence of the validity of our framework.
 
\begin{table}[!ht]\small
  \centering 
  \vspace*{-0.6em}
  \caption{Performance with unknowns manually removed% for leave one out strategy.
  	  \vspace*{-0.8em}}
  	\label{tab:nounkn}
  	\scalebox{0.8}{
  	\begin{tabular}{c|cc|cc} 
  		\hline
  		\multirow{2}{*}{Model} & \multicolumn{2}{c|}{Precision} & 
...
On average, \serene{} yields very good results already 
